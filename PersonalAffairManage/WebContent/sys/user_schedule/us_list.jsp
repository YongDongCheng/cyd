...
<body>
	...
	<!-- 过滤条件 -->
	<div id="QueryArea">
		<form action="${pageContext.request.contextPath }/us?method=search" method="get">
			<input type="hidden" name="method" value="search">
			创建者名字:<input type="text" name="us_create_name" placeholder="请输入创建者名称"> 
	...
	<div id="MainArea">
		<table class="MainArea_Content" align="center" cellspacing="0"
			...
			<tbody id="TableData">
				<c:choose>
					<c:when test="${not empty requestScope.listUS}">

						<c:forEach var="us" items="${requestScope.listUS}">
					
							<tr>
								<td>${us.id}</td>
								<td>${us.us_create_name}</td>
								<td>${us.us_create_identicard_id}</td>
								<td>${us.us_content}</td>
								<td>${us.us_place}</td>
								<td>${us.us_info}</td>
								<td><a
									href="${pageContext.request.contextPath }/us?id=${us.id}&method=viewUpdate"
									class="FunctionButton">更新</a> <a
									href="${pageContext.request.contextPath }/us?id=${us.id}&method=delete"
		...
		<div id="TableTail" align="center">
			<div class="FunctionButton">
				<a href="${pageContext.request.contextPath }/sys/user_schedule/us_save.jsp">添加</a>
			</div>
		</div>
	</div>
</body>
