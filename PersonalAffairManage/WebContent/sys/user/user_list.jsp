...
<body>
	...
	<!-- 过滤条件 -->
	<div id="QueryArea">
		<form action="${pageContext.request.contextPath}/user?method=search" method="get">
			<input type="hidden" name="method" value="search">
			用户名：<input type="text" name="name" placeholder="请输入用户名称"> 
	...
	<div id="MainArea">
				...
				<c:choose>
					<c:when test="${not empty requestScope.listUser}">
						...
						<c:forEach var="user" items="${requestScope.listUser}">
							...
							<tr>
								<td>${user.identicard_id}</td>
								<td>${user.name}</td>
								<td>${user.age}</td>
								<td>${user.sex}</td>
								<td>${user. salary}</td>
								<td>${user.marry_condition}</td>
								<td>${user.job}</td>
								<td>${user.address}</td>
								<td>${user.phone}</td>
								<td><a
									href="${pageContext.request.contextPath }/user?identicard_id=${user.identicard_id}&method=viewUpdate"
									class="FunctionButton">更新</a> <a
									href="${pageContext.request.contextPath }/user?identicard_id=${user.identicard_id}&method=delete"
		...
		<div id="TableTail" align="center">
			<div class="FunctionButton">
				<a href="${pageContext.request.contextPath }/sys/user/user_save.jsp">添加</a>
			</div>
		</div>
	</div>
</body>
