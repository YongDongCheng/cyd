...
<body>
	...
	<!-- 过滤条件 -->
	<div id="QueryArea">
		<form action="${pageContext.request.contextPath }/pgp?method=search" method="get">
			<input type="hidden" name="method" value="search">
			创建者名字:<input type="text" name="pgp_create_name" placeholder="请输入创建者名称"> 
	...
	<div id="MainArea">
		<table class="MainArea_Content" align="center" cellspacing="0"
			...
			<tbody id="TableData">
				<c:choose>
					<c:when test="${not empty requestScope.listGP}">

						<c:forEach var="pgp" items="${requestScope.listGP}">
					
							<tr>
								<td>${pgp.id}</td>
								<td>${pgp.pgp_create_name}</td>
								<td>${pgp.pgp_create_identicard_id}</td>
								<td>${pgp.getmoney}</td>
								<td>${pgp.paymoney}</td>
								<td>${pgp.pgp_info}</td>
								<td><a
									href="${pageContext.request.contextPath }/pgp?id=${pgp.id}&method=viewUpdate"
									class="FunctionButton">更新</a> <a
									href="${pageContext.request.contextPath }/pgp?id=${pgp.id}&method=delete"
		...
		<div id="TableTail" align="center">
			<div class="FunctionButton">
				<a href="${pageContext.request.contextPath }/sys/personal_gp/pgp_save.jsp">添加</a>
			</div>
		</div>
	</div>
</body>
