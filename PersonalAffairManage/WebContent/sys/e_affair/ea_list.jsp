...
<body>
	...
	<!-- 过滤条件 -->
	<div id="QueryArea">
		<form action="${pageContext.request.contextPath }/ea?method=search" method="get">
			<input type="hidden" name="method" value="search">
			创建者名字:<input type="text" name="ea_create_name" placeholder="请输人创建者名称"> 
	...
	<div id="MainArea">
		<table class="MainArea_Content" align="center" cellspacing="0"
			...
			<tbody id="TableData">
				<c:choose>
					<c:when test="${not empty requestScope.listAffair}">

						<c:forEach var="ea" items="${requestScope.listAffair}">
						
							<tr>
								<td>${ea.id}</td>
								<td>${ea.ea_create_name}</td>
								<td>${ea.ea_create_identicard_id}</td>
								<td>${ea.ea_object}</td>
								<td>${ea.ea_place}</td>
								<td>${ea.ea_content}</td>
								<td>${ea.ea_info}</td>
								...
								<td><a
									href="${pageContext.request.contextPath }/ea?id=${ea.id}&method=viewUpdate"
									class="FunctionButton">更新</a> <a
									href="${pageContext.request.contextPath }/ea?id=${ea.id}&method=delete"
		...
		<div id="TableTail" align="center">
			<div class="FunctionButton">
				<a href="${pageContext.request.contextPath }/sys/e_affair/ea_save.jsp">添加</a>
			</div>
		</div>
	</div>
</body>
